...
- `.github/workflows/backend-deploy.yml` - backend CI/deploy pipeline that tests, Buildx-builds the API image, pushes GHCR, runs migrations noninteractively, restarts the GCP VM stack, and waits for a ready API health response.
