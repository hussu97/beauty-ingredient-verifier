...
- `shared/profile-options.json` - source-backed clinical profile vocabulary used by backend rule matching; the frontend vendors the same vocabulary under `frontend/src/data/` for Vercel builds, and the backend vendors it under `backend/shared/` for Docker builds rooted at `backend/`.
...
Profile inputs are controlled custom dropdowns sourced from the vendored frontend copy of `shared/profile-options.json`. Free-text profile values from older local storage are canonicalized through backend aliases when possible and unsupported values are dropped rather than sent to risk rules.
...
The directory PLP API keeps listing work bounded: it paginates with indexed product fields and computes compact risk summaries only for the returned page, avoiding a full catalog product/ingredient/risk-rule aggregate at request time.
