...
9. `sync-local-to-prod` pushes local SQLite catalog/source/embedding data to production Postgres through explicit idempotent upserts, records applied runs in `sync_runs`, refreshes the pgvector image index, and refuses to sync runtime scan/evaluation tables.
...
The `/directory` route is a user-facing PLP for brand/category browsing. It loads brand or category groups, sends debounced group search text to the backend so matches outside the default high-volume group list are discoverable, and does not auto-rank the first search result until the user selects a group. The backend uses database-side coarse risk ordering to fetch a bounded result window, then runs profile-aware rule evaluation only on that window before returning `items/total/limit/offset`. Product detail pages reuse the scanner product-risk panel and include an editable profile form; changing the profile automatically refreshes the product warning summary.
...
Directory product ranking uses the same rule matcher as product risk evaluation on a bounded page window, but does not persist `risk_evaluations`; persisted evaluations are reserved for explicit product evaluation flows.
