...
9. `sync-local-to-prod` pushes local SQLite catalog/source/embedding data to production Postgres through dependency-ordered idempotent upserts, records applied runs in `sync_runs`, uses timestamp deltas after a safe full bootstrap, refreshes the pgvector image index, and refuses to sync runtime scan/evaluation tables.
